...
app.get("/api/books", function (req, res) {
    database.collection(BOOKS_COLLECTION).find({}).toArray(function (error, data) {
        if (error) {
            manageError(res, err.message, "Failed to get contacts.");
        } else {
            res.status(200).json(data);
...
app.get('/api/book/:_id', (req, res) => {
	database.collection(BOOKS_COLLECTION).findById({ _id: new ObjectID(req.params.id) }).toArray(function (error, data) {
        if (error) {
            manageError(res, err.message, "Failed to get contacts.");
        } else {
            res.status(200).json(data);
